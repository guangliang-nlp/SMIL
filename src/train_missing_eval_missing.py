...
def parse_args():
    ...
    parser.add_argument('--per_class_num', default=15, type=int,
                        help='per_class_num')  # 15 * 10 = 150 sound data available, total 1500 sound data
...
def main(args):
    ...
    meta_test_dataset = SoundMNIST(img_root=args.image_root, sound_root=args.sound_root,
                                   per_class_num=args.per_class_num, train=False)
    meta_test_loader = DataLoader(meta_test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=0,
                                  pin_memory=True)
    ...
    image_sound_extractor = SoundLenet5New(i_extractor, s_extractor, extractor_grad=False).to(
        device)  # multimodal fusion model

    ckpt_image_sound = torch.load(
        path.join(args.soundmnist_model_path, args.soundmnist_model_name))  # load pre-trained weight
    ...
    print("==> Total parameters (reference): {:.2f}M".format(
        sum(p.numel() for p in image_sound_extractor.parameters()) / 1000000.0))
    ...
    for iterate in range(args.iterations):
        ...
        if iterate == 0:
            test_image_acc_i = eval_image_test(meta_test_loader, image_sound_extractor, encoder, sound_mean, device,
                                               writer)

            mtr_image_acc_i = eval_image_train(meta_train_loader, image_sound_extractor, encoder, sound_mean, device,
                                               writer)
            mval_image_acc_i = eval_image_test(meta_val_loader, image_sound_extractor, encoder, sound_mean, device,
                                               writer)
            torch.set_grad_enabled(True)
            print('Initial Model  ACC: mtr image acc:{:.4f}, mval image acc:{:.4f}, image-only test acc:{:.4f}'.format(
                mtr_image_acc_i, mval_image_acc_i, test_image_acc_i))

        meta_train_batch = next(iter(meta_train_loader))

        meta_val_batch = next(iter(meta_val_loader))

        glob_step = meta_training(args, meta_train_batch, meta_val_batch, sound_mean, image_sound_extractor, encoder,
                                  criterion_meta_train, criterion_meta_val, optimizer_image_sound, optimizer_encoder,
                                  device, writer, iterate, args.iterations)
        ...
        if (iterate) % 200 == 0:
            ...
            test_image_acc = 0.
            for i in range(30):
                test_image_acc += eval_image_test(meta_test_loader, image_sound_extractor, encoder, sound_mean, device,
                                                  writer)
            test_image_acc = test_image_acc / 30

            mtr_image_acc = eval_image_train(meta_train_loader, image_sound_extractor, encoder, sound_mean, device,
                                             writer)
            mval_image_acc = eval_image_test(meta_val_loader, image_sound_extractor, encoder, sound_mean, device,
                                             writer)
            torch.set_grad_enabled(True)
            print('Iteration:[{}/{}], mtr image acc:{:.6f}, mval image acc:{:.6f}, image-only test acc:{:.6f}'.format(
                iterate, args.iterations, mtr_image_acc, mval_image_acc, test_image_acc))
        ...
        if (iterate) >= 1200:
            ...
            if best_val_acc is None or best_val_acc < test_image_acc:
                ...
                save_ckpt_classifier(
                    {'iterate': iterate, 'lr': args.lr, 'state_dict': image_sound_extractor.state_dict(),
                     'optimizer': optimizer_image_sound.state_dict()}, ckpt_dir_path, iteration=iterate,
                    best_acc=best_val_acc)
                save_ckpt_inferNet(
                    {'iterate': iterate, 'lr': args.lr, 'step': glob_step, 'state_dict': encoder.state_dict(),
                     'optimizer': optimizer_encoder.state_dict()}, ckpt_dir_path, iteration=iterate)
    writer.close()


def meta_training(args, meta_train_batch, meta_val_batch, sound_mean, image_sound_extractor, encoder,
                  criterion_meta_train, criterion_meta_val, optimizer_image_sound, optimizer_encoder, device, writer,
                  iterate, total_iterate):
    ''' train one epoch'''
    ...
    for idx in range(args.inner_loop):
        ...
        pred_meta_train_noised, _, _, _ = image_sound_extractor.functional(params_star, True)(meta_train_label,
                                                                                              meta_train_image,
                                                                                              sound=None,
                                                                                              encoder=encoder,
                                                                                              sound_mean=sound_mean,
                                                                                              noise_layer=['fc0', 'fc1',
                                                                                                           'fc2'],
                                                                                              meta_train=True,
                                                                                              mode='one')
        ...
        grads = torch.autograd.grad(loss_meta_train, params_star, allow_unused=True,
                                    create_graph=True)  # create_graph=True: allow second order derivative
        ...
        for i in range(len(params_star)):
            if i <= 7 or i >= 26:  # not update the sound branch
                if grads[i] is not None:  # unused parameters have no gradient
                    params_star[i] = (
                                params_star[i] - args.lr_inner * (0.1 ** (iterate // 1000)) * grads[i]).requires_grad_()

    pred_meta_val_noised, f_meta_val_noised1, f_meta_val_noised2, sound_mean_val_noised = image_sound_extractor.functional(
        params_star, True)(meta_val_label, meta_val_image, sound=None, encoder=encoder, sound_mean=sound_mean,
                           noise_layer=['fc0', 'fc1', 'fc2'], meta_train=False, mode='one')

    pred_meta_val_clean, f_meta_val_clean1, f_meta_val_clean2, sound_mean_val_clean = image_sound_extractor.functional(
        params_star, True)(meta_val_label, meta_val_image, sound=meta_val_sound, mode='two')

    sound_mean_val_mse = mse_loss(sound_mean_val_clean, sound_mean_val_noised)

    loss_meta_val = criterion_meta_val(sound_mean_val_clean, sound_mean_val_noised, f_meta_val_clean1,
                                       f_meta_val_noised1, f_meta_val_clean2, f_meta_val_noised2, pred_meta_val_noised,
                                       pred_meta_val_clean, meta_val_label)
...
def eval_image_test(test_loader, image_sound_extractor, encoder, sound_mean, device, writer):
    ...
    for i, batch in enumerate(test_loader):
        ...
        outputs, _, _, _ = image_sound_extractor(labels, images, sound=None, encoder=encoder, sound_mean=sound_mean,
                                                 noise_layer=['fc0', 'fc1', 'fc2'], meta_train=False, mode='one')
